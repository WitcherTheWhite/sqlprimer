use serde::{
    de::{self, DeserializeSeed, EnumAccess, IntoDeserializer, SeqAccess},
    ser, Serialize,
};
...
impl<'a> ser::Serializer for &'a mut Serializer {
    ...
    fn serialize_unit_struct(self, __name: &'static str) -> Result<()> {
        todo!()
    }
    ...
    fn serialize_unit_variant(
        ...
        self.output.extend(u8::try_from(_variant_index));
        Ok(())
    }

    fn serialize_newtype_struct<T>(self, __name: &'static str, _value: &T) -> Result<()>
    where
        T: ?Sized + ser::Serialize,
    {
        todo!()
    ...
    where
        T: ?Sized + ser::Serialize,
    {
        self.serialize_unit_variant(_name, _variant_index, variant)?;
        value.serialize(self)?;
        Ok(())
    }

    fn serialize_seq(self, __len: Option<usize>) -> Result<Self::SerializeSeq> {
        Ok(self)
    }
...
impl<'de> Deserializer<'de> {
    fn take_bytes(&mut self, _len: usize) -> &[u8] {
        let bytes = &self.input[.._len];
        self.input = &self.input[_len..];
        bytes
    }
...
impl<'de> EnumAccess<'de> for &mut Deserializer<'de> {
        ...
        V: DeserializeSeed<'de>,
    {
        ...
        let _variant_index: Result<_> = seed.deserialize(index.into_deserializer());
        Ok((_variant_index?, self))
    }
}
...
mod tests {
    ...
    fn test_encode_prefix() {
        ...
        ser_cmp(
            MvccKeyPrefix::Version(b"abc".to_vec()),
            vec![3, 97, 98, 99, 0, 0],
        );
    }
